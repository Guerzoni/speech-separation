...
from DataLoaders import DataLoaders, Datasets, make_dataloader
import sys
...
class ConvTasNet(nn.Module):
    ...
    def forward(self, x):
        ...
        s = [torch.unsqueeze(s[i], dim=0) for i in range(self.num_spks)]
        s = [torch.unsqueeze(s[i], dim=0) for i in range(self.num_spks)]
...
def test_convtasnet():
    ...
    for i, egs in enumerate(dataloaders):
        x1 = egs[0]['mix'].squeeze()
        break
    print('x1', x1)
    nnet = ConvTasNet(N=1024)
    s = nnet(x1)
